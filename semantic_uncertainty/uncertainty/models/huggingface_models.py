...
class HuggingfaceModel(BaseModel):
    ...
    def predict(self, input_data, temperature, return_full=False):
        ...
        if self.stop_sequences is not None:
            for stop in self.stop_sequences:
                if answer.endswith(stop):
                    stop_at = len(answer)
                    stop_at_2 = len(answer) - len(stop)
                    sliced_answer = answer[:stop_at_2]
                    break
            if not all([stop not in sliced_answer for stop in self.stop_sequences]):
                error_msg = 'Error: Stop words not removed successfully!'
                error_msg += f'Answer: >{answer}< '
                error_msg += f'Sliced Answer: >{sliced_answer}<'
                if 'falcon' not in self.model_name.lower() and 'llama-3.3-70b' not in self.model_name.lower():
        ...
        if len(log_likelihoods) == 1:
            ...
            log_likelihoods = log_likelihoods
            # log_likelihoods = log_likelihoods[:min(n_generated+1, len(log_likelihoods))]
