...
def get_p_ik(aim_run:Run, train_embeddings, is_false, eval_embeddings=None, eval_is_false=None):
    ...
    metrics_convergence_dict = {**metrics, **convergence}
    metrics_convergence_dict_processed = {}
    for k,v in metrics_convergence_dict.items():
        if type(v) == np.float32 or type(v) == np.float64 or type(v) == np.int32 or type(v) == np.int64 or type(v) == np.uint32 or type(v) == np.uint64 or type(v) == np.bool:
            metrics_convergence_dict_processed[k] = v.item()
        else:
            metrics_convergence_dict_processed[k] = v
    
    logging.info(str(metrics_convergence_dict_processed))
    aim_run['metrics_convergence'] = metrics_convergence_dict_processed
