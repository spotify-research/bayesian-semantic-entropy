...
class EntailmentLLM(BaseEntailment):
    ...
    def init_prediction_cache(self, entailment_cache_id):
        ...
        aim_run = Run(hash=entailment_cache_id)
        directory = os.path.join(aim_run.repo.path, aim_run.hash)

        # run.file(self.entailment_file).download(
        #     replace=True, exist_ok=False, root=wandb.run.dir)

        with open(f'{directory}/{self.entailment_file}', "rb") as infile:
            return pickle.load(infile)

    def save_prediction_cache(self):
        # Write the dictionary to a pickle file.
        utils.save(self.prediction_cache, self.entailment_file, self.aim_run)
...
def get_semantic_ids(strings_list_in, model, strict_entailment=False, example=None):
    """Group list of predictions into semantic meaning."""

    def are_equivalent(text1 : str, text2 : str):
        
        # if text1.lower() == text2.lower():
        #     return True # identical strings are always equivalent
        ...
        #logging.info(f"Checking equivalence of {text1} and {text2}, result {semantically_equivalent}.")
        return semantically_equivalent

    # Force exact same strings into the same bucket
    strings_list = list(set(strings_list_in)) 
    string_to_index = { s:i for i,s in enumerate(strings_list) }
    list_index_to_set_index = {li: string_to_index[strings_list_in[li]] for li in range(len(strings_list_in))}
    ...
    semantic_set_ids_list = [semantic_set_ids[list_index_to_set_index[li]] for li in range(len(strings_list_in))]

    #logging.info(f"Assigned semantic classes: {str(list(zip(semantic_set_ids_lst, strings_list)))}.")

    return semantic_set_ids_list
