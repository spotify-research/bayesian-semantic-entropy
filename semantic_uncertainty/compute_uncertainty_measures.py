...
def main(args, aim_run_hash = None):

    if not aim_run_hash is None:
        aim_run = Run(run_hash=aim_run_hash)
    else:
        aim_run = Run()
    ...
    if args.train_wandb_runid != args.eval_wandb_runid:
        assert(False, "Not implemented with AIM.")
        # logging.info(
        ...
        if args.compute_p_ik or args.compute_p_ik_answerable:
            with open(directory + "/train_generations.pkl", 'rb') as infile:
                train_generations = pickle.load(infile)

    #wandb.config.update({"is_ood_eval": is_ood_eval}, allow_val_change=True)
    aim_run['is_ood_eval'] = is_ood_eval
    ...
    if args.compute_p_true_in_compute_stage:
        ...
        aim_run.track(len_p_true, name='len_p_true')
    ...
    with open(directory + '/uncertainty_measures.pkl', "rb") as infile:
        result_dict = pickle.load(infile)
    result_dict['semantic_ids'] = []

    with open(directory + '/validation_generations.pkl', 'rb') as infile:
    ...
    for idx, tid in enumerate(validation_generations):
        ...
        count += 1
        if count >= int(args.num_eval_samples):
    ...
    if args.compute_p_ik:
        ...
        p_ik_predictions = get_p_ik(aim_run=aim_run,
    ...
    utils.save(result_dict, 'uncertainty_measures.pkl', aim_run)
